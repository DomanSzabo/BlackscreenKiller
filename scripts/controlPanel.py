...
program_to_terminate = "nfscblscr.exe"
...
class ControlPanel(tk.Tk):
    ...
    def create_buttons(self):
        ...
        for script_name, command in self.scripts.items():
            button = tk.Button(self, text=script_name, command=lambda cmd=command: (
            executer.run_wo_interrupt(cmd), exit(), sleep(1)), **button_style)
